...
![image-20250804113917194](./%E9%A9%B1%E5%8A%A8.assets/image-20250804113917194.png)
...
对于不知道如何调用的函数，可以去查看源码，或者通过观察其他函数如何调用来判断，这里在执行的时候出现了一个新的结构`_RTL_PROCESS_MODULES`，我们去查看该结构。

![image-20250820141352904](./%E9%A9%B1%E5%8A%A8.assets/image-20250820141352904.png)

或者通过这里也可以看到，其源码在调用时也是二次调用进行内存分配。首先在一次调用时，返回其所需要的大小，二次调用重新拿到分配。

并且，在这里也能看到两个结构。

![image-20250820144037122](./%E9%A9%B1%E5%8A%A8.assets/image-20250820144037122.png)
...
在查看其调用时我们可以看到，`MiFindExportedRoutineByName`函数的参数有两个一个是模块的名字，另外一个是目标函数，也就是说我们在调用时如果查找非nt和hal模块那么在传入字符时需要传入模块名和函数名。

![image-20250821174432329](./%E9%A9%B1%E5%8A%A8.assets/image-20250821174432329.png)
...
还有一种思路是尝试去查询是否有`RtlFindExportedRoutineByName()`这个函数，如果找得到就认为是Win10，否则就按照Win7去处理。
