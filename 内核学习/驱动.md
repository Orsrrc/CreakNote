...
例如原本是FDO.1、PDO.1那么创建一个新的FDO.2后，FDO.2就会先FDO.1收到消息并决定消息的派发，此时FDO.1就是上一个过滤器驱动对象，那么此时就会返回给你
...
1.将过滤器附加到驱动上，将三环发来的消息拦截，不派发给驱动程序
...
2.过滤器往下派发消息，如果发送过来的是READ就往下派发，如果是TEST就拦截
...
3.将消息派发下去后，驱动程序把数据进行处理，处理完成以后的结果返回到过滤器里面
...
case WRITE:
	{
        ...
        //将当前栈的地址复制到下一个栈里面去
        //三环发送数据到过滤器驱动的栈里面去，而驱动程序对三环发过来的数据有一个写入的操作，我们将栈复制到下一个栈栈里面去，那么驱动操作的就是三环发过来的数据。
		IoSetCompletionRoutine(Irp, ioCmpCallback, NULL, TRUE, FALSE, FALSE); //IO完成后回调
       ...
       //这里执行的顺序是当IoCallDriver函数完成了以后，由于设定了IO完成后回调，因此会调用ioCmpCallback函数执行
		if (st == STATUS_PENDING)
		{
